...
from nfelib.nfe.bindings.v4_0.dfe_tipos_basicos_v1_00 import Tcibs, Tred, TtribNfe
...
> <det>
    ...
    def _export_field(self, xsd_field, class_obj, member_spec, export_value=None):
        ...
        if xsd_field == "nfe40_IBSCBS":
            ...
            # Build gIBSUF
            gRed = (
                Tred(pRedAliq=f"{self.ibs_reduction:.4f}", pAliqEfet=f"{p_ibs_uf:.4f}")
            ...
            # Build gIBSMun
            gRed = (
                Tred(pRedAliq=f"{self.ibs_reduction:.4f}", pAliqEfet=f"{p_ibs_mun:.4f}")
                if self.ibs_reduction
                else None
            )
            ...
            # Build gCBS
            gRed = (
                Tred(pRedAliq=f"{self.cbs_reduction:.4f}", pAliqEfet=f"{p_cbs:.4f}")
